...
class EmailvalidatorValidationException implements Exception {
  final List<String> errors;

  EmailvalidatorValidationException(this.errors);

  @override
  String toString() => 'EmailvalidatorValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://emailvalidator.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [email] (required) - The email address to validate (e.g., support@myspace.com) [format: email]
class EmailvalidatorClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'email': ValidationRule(type: 'string', required: true, format: 'email'),
  };

  EmailvalidatorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/emailvalidator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [EmailvalidatorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw EmailvalidatorValidationException(errors);
    }
  }

  /// Execute a request to the Email Validator API.
  ///
  /// Parameters are validated before sending the request.
  Future<EmailvalidatorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw EmailvalidatorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return EmailvalidatorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw EmailvalidatorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw EmailvalidatorException('Resource not found');
      } else {
        throw EmailvalidatorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is EmailvalidatorException) rethrow;
      throw EmailvalidatorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Email Validator API client.
class EmailvalidatorException implements Exception {
  final String message;

  EmailvalidatorException(this.message);

  @override
  String toString() => 'EmailvalidatorException: $message';
}
